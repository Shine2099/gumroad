...
class CreatorAnalytics::ProductPageViews
  def initialize(user:, products:, dates:)
    ...
    @query = {
      bool: {
        ...
        must: [{ range: { timestamp: { time_zone: @timezone, gte: @dates.first.to_s, lte: @dates.last.to_s } } }]
      }
    }
  end
  ...
  def by_product_and_date
    sources = [
      ...
      { date: { date_histogram: { time_zone: @timezone, field: "timestamp", calendar_interval: "day", format: "yyyy-MM-dd" } } }
    ]
