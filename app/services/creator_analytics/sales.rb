...
class CreatorAnalytics::Sales
  ...
  def initialize(user:, products:, dates:)
    ...
    @query[:bool][:must] << { range: { created_at: { time_zone: @timezone, gte: @dates.first.to_s, lte: @dates.last.to_s } } }
  end

  def by_product_and_date
    sources = [
      { product_id: { terms: { field: "product_id" } } },
      { date: { date_histogram: { time_zone: @timezone, field: "created_at", calendar_interval: "day", format: "yyyy-MM-dd" } } }
    ]
