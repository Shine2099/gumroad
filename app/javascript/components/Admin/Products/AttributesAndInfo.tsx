...
const ProductAttributesAndInfo = ({ productData }: Props) => {
  ...
  return hasNoAttributes && preorder == null && !should_show_sales_count && !has_stream_only_files ? (
    <Alert color="info">No additional details.</Alert>
  ) : (
    <div className="product-info grid gap-4">
      ...
      {preorder != null ? (
        <>
          <Alert color="info">Available on {preorder.release_date_fmt}</Alert>
      ...
      {has_stream_only_files ? (
        <>
          <Alert color="info">Watch link provided after purchase</Alert>
