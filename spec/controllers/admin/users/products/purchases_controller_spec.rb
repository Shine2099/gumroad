...
describe Admin::Users::Products::PurchasesController do
  render_views
  ...
  describe "GET index" do
    it "returns a JSON payload with the purchases" do
      get :index, params: { user_id: affiliate_user.id, product_id: product.id }
    ...
    context "with pagination" do
      ...
      before do
        get :index, params: { user_id: affiliate_user.id, product_id: product.id, page:, per_page: }
      end
      ...
      it "returns the correctly paginated purchases (page 2)" do
        get :index, params: { user_id: affiliate_user.id, product_id: product.id, page: 2, per_page: }
        expect(response).to be_successful
